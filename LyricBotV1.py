...
async def get_song(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    def get_lyric():
        url = f"https://www.letras.mus.br/{artist}/{songname}.html"
    ...
    #def cover():
        #url = f"https://www.azlyrics.com/lyrics/{artist.replace('-', '')}/{songname.replace('-', '')}.html"
        #response = requests.get(url)
        #soup = BeautifulSoup(response.text, "html.parser")
        #cover = soup.find("div", {"class": "songinalbum_title"})
        #return f"https://www.azlyrics.com/{cover.find('img').attrs['src']}"
